...
func (c *Consumer) connect() error {
	...
	if err != nil {
		return errors.Wrap(err, "Channel error:")
	}
	...
	if err = c.Channel.ExchangeDeclare(
		...
		return errors.Wrap(err, "Exchange declare error")
	}
...
func handle(deliveries <-chan amqp.Delivery, done chan error) {
	for d := range deliveries {
		...
		err := d.Ack(false)
		if err != nil {
			errors.Wrap(err, "Queue Declare error")
		}
	}
...
func (c *Consumer) declare() (<-chan amqp.Delivery, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "Queue Declare error")
	}

	// Число сообщений, которые можно подтвердить за раз.
	err = c.Channel.Qos(50, 0, false)
	if err != nil {
		return nil, errors.Wrap(err, "Error setting qos:")
	}
	...
	if err = c.Channel.QueueBind(
		...
		return nil, errors.Wrap(err, "Queue Bind error:")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "Queue Consume error:")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "Queue Publish error: :")
	}
...
func (c *Consumer) Handle(ctx context.Context, threads int) error {
	...
	for {
		...
		if <-c.done != nil {
			...
			if err != nil {
				return errors.Wrap(err, "Reconnecting error:")
			}
		}
